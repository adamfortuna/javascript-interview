...
// To run: mocha -w tests/1_reverse_test.js
...
describe('reverse', function() {
  ...
  describe('with an array longer than 1', function() {
    ...
    it('When called, the array should have the same length as passed in array. @reverse', function() {
      ...
      assert.equal(reverseResultLength, longArray.length, "We tried calling your function with an array with `"+longArray.length+"` elements but your reverse function returned an array with `"+reverseResultLength+"` item(s). @reverse");
    });

    it('The `reverse` function should return the reversed array. @reverse', function() {
      var reverseResult = reverse(longArray),
          correct = [5,4,3,2,1];
      assert.deepEqual(reverseResult, correct, "We called your reverse function passing in `"+JSON.stringify(longArray)+" and expected to get back `" + JSON.stringify(correct) + "`, but it returned `"+JSON.stringify(reverseResult)+"`. @reverse");
    });
  });
});
